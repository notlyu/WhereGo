...
export function MapPage() {
  ...
  return (
    // Высота считается от `dvh`, а не `vh`: в Safari на айфоне `vh` меряет
    // экран без свёрнутой адресной строки, и низ карты — вместе с кнопкой
    // списка — уезжает под панель браузера.
    //
    // Отрицательный отступ снизу гасит `padding-bottom` у `main`: там место
    // под плавающую навигацию, а карта и так заканчивается над ней. Без этого
    // под картой оставалась чёрная полоса на 128px, которую можно прокрутить.
    <div className="relative -mb-[calc(128px+env(safe-area-inset-bottom))] h-[calc(100dvh-96px-env(safe-area-inset-bottom))]">
      {canvas}
      {errorBanner}
      {locateButton}
