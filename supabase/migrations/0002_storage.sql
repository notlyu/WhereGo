...
-- Проверить наличие бакета отсюда нельзя: `storage.buckets` закрыта RLS и на
-- чтение — запрос вернёт пусто даже для существующего бакета. Политикам ниже
-- это и не нужно: `bucket_id = 'photos'` — сравнение строк, а не внешний ключ.
