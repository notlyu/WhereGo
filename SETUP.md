...
> [!danger] Любая ошибка про права — сначала проверьте роль
> `permission denied`, `must be owner of table`, `violates row-level security` —
> в девяти случаях из десяти это не про права как таковые, а про переключатель
> роли над полем ввода SQL Editor. Он запоминается между запросами.
>
> ```sql
> select current_user;
> ```
>
> Должно быть `postgres`. Если там `authenticated` или `anon` — вернуть на
> `postgres` и повторить запрос. Под ограниченной ролью не выйдет ни изменить
> таблицу, ни создать политику, ни прочитать `auth.users`.
...
- Type: **HTTP(s)**
- Interval: **5 минут**
- URL — **не корневой адрес проекта**, а запрос к таблице:

```
https://<ref>.supabase.co/rest/v1/categories?select=id&limit=1&apikey=<publishable-ключ>
```

Проверить, что адрес верный, можно заранее:

🖥 **Терминал**

```bash
set -a && . ./.env.local && set +a && curl -s -o /dev/null -w '%{http_code}\n' "$VITE_SUPABASE_URL/rest/v1/categories?select=id&limit=1&apikey=$VITE_SUPABASE_ANON_KEY"
```

Должно вывести `200`.

> [!warning] Почему не корневой адрес
> `https://<ref>.supabase.co/` отдаёт **404** — `{"error":"requested path is invalid"}`.
> Монитор считал бы сайт упавшим и слал письма о сбое.
>
> Хуже другое: такой запрос обрабатывает шлюз, до Postgres он не доходит.
> Смысл пинга — создавать активность **базы**, иначе проект всё равно уснёт
> через 7 дней, а монитор будет исправно показывать «всё хорошо».
>
> Запрос к `/rest/v1/categories` выполняет настоящий SQL. Ответ `[]` — это
> не ошибка: RLS не отдаёт данные неавторизованному, но обращение к базе
> состоялось, а больше ничего и не нужно.

Ключ в адресе — тот же publishable, что уже лежит в JS-файлах сайта и виден
любому посетителю. Секретный ключ сюда не подставлять.
...
| Поле | Значение |
|---|---|
| Bucket name | `wherego-photos` |
| Location | `EEUR` (Eastern Europe) — ближайший регион |
| Storage class | Standard |

> [!note] Может попросить карту
> Cloudflare иногда требует привязать способ оплаты, чтобы включить R2, даже
> ради бесплатного объёма. Списаний в пределах 10 ГБ не будет. Если просит и
> это не подходит — скажите, обсудим, чем заменить.

### 11.2 Публичный доступ — НЕ включать

🖱 В настройках бакета есть **Public Development URL** (`r2.dev`) и Custom
Domains. **Оставьте выключенным.**

Фото отдаёт наш Worker: он и так стоит перед приложением, а бакет остаётся
закрытым для всех, кроме него. Заодно это снимает ограничение скорости, которое
Cloudflare накладывает на `r2.dev`, и не требует своего домена.

### 11.3 Привязать бакет к Worker

🖱 **Compute (Workers) → `wherego` → Settings → Bindings → Add → R2 bucket**

| Поле | Значение |
|---|---|
| Variable name | `PHOTOS` |
| R2 bucket | `wherego-photos` |

Имя переменной именно `PHOTOS` — на него ссылается код Worker'а.

Привязку дублирует `wrangler.jsonc` в репозитории, так что при следующем деплое
она пропишется сама. Добавить её в дашборде стоит, чтобы проверить, что бакет
виден Worker'у, до того как поедет код.

### 11.4 Проверить

🖥 **Терминал** — после того как код Worker'а будет задеплоен:

```bash
curl -s -o /dev/null -w '%{http_code}\n' https://wherego.stanislav-biktimirovscky.workers.dev/api/photos/health
```

Ожидается `200`. Пока Worker не задеплоен — `404`, это нормально.
...
> [!note] Почему через интерфейс, а не миграцией
> Интерфейс работает независимо от того, какая роль выбрана в SQL Editor, и
> сразу показывает результат в списке политик — ошибиться труднее.
>
> Из SQL это тоже может получиться, но `storage.objects` принадлежит роли
> `supabase_storage_admin`, так что упереться в `must be owner of table objects`
> вполне реально даже под `postgres`. Проверять не стали: политики уже стоят.
